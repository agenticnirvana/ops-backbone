...
const LEARNING_PHASES = [
  ...
  { n: 3, name: 'Evaluation', question: 'How do you trust it before/after deploy?', tools: '{llmops} · MLflow · OTEL', color: 'violet' },
...
const LEARNING_STEPS = [
  ...
  },
  {
    ...
    happens: '{llmops} records every graph span. MLflow eval gate scores golden alerts. OTEL exports standard traces.',
    expect: 'Evaluation → Agent Analytics + {llmops} Trace. Scores tab shows pipeline_latency_ms, llm_call_count.',
  ...
  },
  {
    id: 'step-3b',
    phase: 'Phase 3',
    title: 'MLflow eval gate',
    subtitle: 'Golden alerts must pass before CI merges',
    happens: 'golden_alerts.json expected runbook IDs checked in MLflow. CI fails if agent retrieves wrong runbook.',
    expect: 'Evaluation → Eval Gate shows pass/fail table for checkout-redis and other fixtures.',
...
const LEARNING_TOPICS = [
  ...
  },
  {
    ...
    icon: '📈',
    oss: ['{llmops}', 'MLflow 2', 'OpenTelemetry Collector', 'Pytest'],
    summary: 'Prove the agent is grounded, fast, and repeatable before trusting automation.',
    blocks: [
      {
        h: '{llmops} — runtime tracing',
        ul: [
          'Every LangGraph node becomes a trace span with inputs/outputs (where safe).',
          'Filter traces by <code>thread_id</code> from agent responses — links UI run → {llmops}.',
          'Custom scores pushed after runs: <code>pipeline_latency_ms</code>, <code>llm_call_count</code>, HITL outcome.',
          'OSS self-hosted {llmops} in compose — no cloud dependency for the course.',
        ],
      },
      {
        h: 'MLflow eval gate',
        ul: [
          'Golden alerts in <code>agent/evals/golden_alerts.json</code> — expected runbook IDs, severity, service names.',
          'CI / Evaluation tab invokes the real agent against fixtures and checks runbook recall + groundedness.',
          'MLflow UI on <code>:5001</code> stores experiment runs; failed eval blocks merge in GitHub Actions workflow.',
